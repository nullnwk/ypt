		...
		var fn       = this,
			...
			seat = (function(seatCharts, seatChartsSettings) {
				return function (setup) {
					var fn = this;
					var text = "";
//					console.log(fn.settings.label);
//					text = fn.settings.label;  //是否显示座号数字
					fn.settings = $.extend({
...
//							val:"123",
		...
		$.each(settings.map, function(row, characters) {
			var hangshu = ""
			hangshu = settings.naming.rows[row];  //左侧行数是否显示
			var $row = $('<div></div>').addClass('seatCharts-row');
			if (settings.naming.left) {
				$row.append(
					$('<div style="margin-right: 5px; "></div>')
						.addClass('seatCharts-cell seatCharts-space size8 size10-xs')
						.text(hangshu)
				);
			}
