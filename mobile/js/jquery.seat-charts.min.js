...
!function (t) {
	t.fn.seatCharts = function (s) {
		...
		var e = this, a = {}, n = [], i = {animate: !1, naming: {top: !0, left: !0, getId: function (t, s, e) {
			return s + "_" + e
...
//				i.settings = t.extend({status: "available", style: "available", data: e.seats[n.character] || {}}, n), i.settings.$node = t("<div></div>"), i.settings.$node.attr({id: i.settings.id, role: "checkbox", "aria-checked": !1, focusable: !0, tabIndex: -1}).text(i.settings.label).addClass(["seatCharts-seat", "seatCharts-cell", "available"].concat(i.settings.classes, "undefined" == typeof e.seats[i.settings.character] ? [] : e.seats[i.settings.character].classes).join(" ")), i.data = function () {
		...
		return e.append(c), t.each(i.map, function (s, c) {
			...
			var hangshu = "";
//			hangshu = i.naming.rows[s]; //最左侧行数，是否隐藏
			i.naming.left && u.append(t("<div style='color: #fff;'></div>").addClass("seatCharts-cell seatCharts-space").text(hangshu)), t.each(c.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (e, c) {
				...
				u.append("_" != d ? function (t) {
					...
					return a[c] = new r({id: c, label: f ? f : t.getLabel(d, t.rows[s], t.columns[e]), row: s, column: e, character: d}), n.push(c), a[c].node()
		...
		}), e.data("seatCharts", {seats: a, seatIds: n, status: function () {
			var s = this;
			...
			var s = this;
			return{seats: [], seatIds: [], length: 0, status: function () {
